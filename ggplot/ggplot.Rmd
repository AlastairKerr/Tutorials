...
Note that each layer you add will inherit the attributes from the first layer. Aesthetics can be overwritten but you may want to assigned them later in the geometries for clarity. Note the following:

```{r}
ggplot(data=iris, aes(x=Sepal.Length, y=Sepal.Width) ) + geom_point(aes(x=Petal.Length, y=Petal.Width)) 
```

Although here is no geometry for the 1st set of data, the labels are used but the data has been overwritten using the aesthetics in the geometry.
...
The latest version of R-studio has some useful addins.  "ggExtra" has the ggMarginal function that allows you to add probability density plots to the margins of the your plot. 
```{r}
library(ggExtra)
ggMarginal(p)
```
